...
|-----|-------|------|--------|----------|------------|---------------|--------|
| ADR-010 | Redis/BullMQ job model and env-var api/microservices worker split are an **inherited upstream as-built constraint** this engagement adopts and works within — not a decision made here. Reclassifies ADR-002 via supersession: the queue topology and worker-split lever are shipped by Immich (`immich-architecture-overview.md:100`, `immich-jobs-and-workers.md:15,22,35`), so ADR-002's no-queue "Option B" was never a live engagement option (retained as context only). The durable facts stay homed as as-is facts in `core/2.ARCH-BASELINE.md` §1.2/§1.3/§1.4 (cited, not restated); the engagement's owned decisions layer on top (worker-split hosting topology; Redis HA → ADR-003). R-01/R-02 unchanged. Takes effect only on architect approval. | 2026-07-16 | proposed | MINOR | ADR-002 | — | Simulated architect (demo) |
| ADR-009 | Isolated `immich-machine-learning` service is an **inherited upstream as-built constraint** this engagement adopts and works within — not a decision made here. Reclassifies ADR-001 via supersession: Immich ships the separate stateless Python/FastAPI + ONNX service called synchronously over HTTP (`immich-architecture-overview.md:88,90,92`), so ADR-001's in-process "Option B" was never a live engagement option (retained as context only). The durable fact stays homed as an as-is fact in `core/2.ARCH-BASELINE.md` §1.2/§1.3 (cited, not restated); the engagement's owned ML decisions layer on top (ADR-004 scale-out + channel hardening). R-03 unchanged. Takes effect only on architect approval. | 2026-07-16 | proposed | MINOR | ADR-001 | — | Simulated architect (demo) |
| ADR-008 | Pilot customer-library onboarding approach: adopt **staged operator-driven CLI ingest (C)** as the pilot default — customer data lands on an operator staging volume, the operator runs `immich upload` into the instance, then staging is retired — with **direct CLI upload (A) allowed for small libraries**. Keeps A's quota-counted, storage-template-governed single storage class, always-on server-side dedup, and idempotent re-run, while decoupling the customer transfer from the instance front door (proxy `50000M`/`600s`) and moving the `--concurrency` arrival-rate lever operator-side (the one lever over the NFR-003 ML backlog — T30). **External-library-based onboarding (B) rejected as pilot default** (quota exemption defeats NFR-005 accounting — R-09; single-user unchangeable ownership; path-removal-deletes hazard) but recorded as an explicit opt-in exception requiring `:ro` and a signed-off import-path config. Consequences cover the OQ#6 arrival-format-mapping gap (operator procedure, formats not invented), the NFR-003 backlog tie (T30 arrival-profile control), staging-capacity implications for ADR-005/E-03, and R-09's shrunk exposure under B-exception-only. Onboarding acceptance figure (ML backlog drained per NFR-003) stays [TBD] pending AI-001 (T28–T31, BASELINE §1.4). | 2026-07-12 | proposed | MAJOR | — | — | Simulated architect (demo) |
...
| ADR-002 | As-built: background work runs on Redis/BullMQ job queues; api + microservices workers co-located in one `immich-server` container by default, splittable via `IMMICH_WORKERS_INCLUDE`/`EXCLUDE` — worker split is the sanctioned scaling lever (R-02); Redis is availability-critical queue state (R-01). | 2026-07-11 | approved | MINOR | — | ADR-010 (pending — proposed, not yet approved) | Simulated architect (demo) |
| ADR-001 | As-built: ML inference isolated in the separate stateless `immich-machine-learning` (Python/FastAPI + ONNX) service, called synchronously over HTTP — enables independent ML scaling/sizing and memory isolation from the serving path (R-03). | 2026-07-11 | approved | MINOR | — | ADR-009 (pending — proposed, not yet approved) | Simulated architect (demo) |
